...
if not vim.lsp.config then
	vim.notify("vim.lsp.config is not available. Please upgrade to Neovim 0.11+", vim.log.levels.ERROR)
	return
end
...
local on_attach = function(client, bufnr)
	...
	-- Disable formatting from tsserver for JS/TS files, use eslint instead
...
-- Server setup ----------------------
-- Configure each LSP server with the new vim.lsp.config API
for lsp, extra in pairs(LSP_servers) do
	-- Merge the common configuration with server-specific settings
	local config = {
		on_attach = on_attach,
	...
	-- Merge server-specific configuration
